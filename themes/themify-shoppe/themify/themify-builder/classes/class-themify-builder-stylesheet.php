...
class Themify_Builder_Stylesheet {
            ...
            if (!empty($rules[$mod_name][$id]) || $is_gradient || $is_cover) {
                ...
                if (in_array($type, array('background-color', 'color'),true)) {
                    ...
                    if ($value !== false) {
                        ...
                        if ( isset( $settings['border-type'] ) && $settings['border-type'] === 'all' ) {
                            if( $type !== 'border-top') {
                                continue;
                            }
                    ...
                    elseif($type === 'font-weight'){
                    	$value = ( 'regular' === $value || 'italic' === $value ) ? 400 : preg_replace('/[^0-9]/', '', $value);
    ...
    public function recursive_style_generator($data, $style_id, $sub_row = '') {
        ...
        foreach ($data as $row_index => $row) {
            ...
            if (!$is_subrow && !empty($row['styling']) && is_array($row['styling'])) {
				...
				$row['styling'] = $this->row_frame_styling_set_data( $row['styling'] );
            ...
            foreach ($row['cols'] as $col_index => $col) {
                ...
                if (!empty($col['styling']) && is_array($col['styling'])) {
					...
					$row['styling'] = $this->row_frame_styling_set_data( $col['styling'] );
	...
	/**
	 * Fixes how responsive Row Frame option is saved, so the CSS is generated properly.
	 *
	 * @return array
	 */
	function row_frame_styling_set_data( $styling ) {
		foreach ( array( 'tablet_landscape', 'tablet', 'mobile' ) as $bp ) {
			if ( isset( $styling[ "breakpoint_{$bp}" ] ) ) {
				foreach ( array( 'top', 'right', 'bottom', 'left' ) as $side ) {
					if ( ! isset( $styling[ "breakpoint_{$bp}" ][ "{$side}-frame_layout" ] ) && isset( $styling[ "{$side}-frame_layout" ] ) ) {
						$styling[ "breakpoint_{$bp}" ][ "{$side}-frame_layout" ] = $styling[ "{$side}-frame_layout" ];
					}
				}
			}
		}
