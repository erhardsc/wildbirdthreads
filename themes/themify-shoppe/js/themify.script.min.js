...
d.hasClass("header-right-pane")){var u=b(".top-icon-wrap, .search-button"),v=b("#mobile-menu");b(v).prepend(b('<div class="header-icons"></div>'));b(".header-icons").append(u)}d.hasClass("header-overlay")&&b(".search-button").appendTo(".top-icon-wrap");(function(a,c){if(a.length&&c.length&&!d.is(".header-left-pane, .header-right-pane, .header-minbar-left, .header-minbar-right")){var e=a.clone(),g=b("#shopdock"),h=g.clone(),f=b("#slide-cart").length,k=g.attr("id"),l=k+"_",n=function(){e.is(":visible")?
(g.attr("id",l).hide(),h.attr("id",k).show()):(g.attr("id",k).show(),h.attr("id",l).hide())};e.addClass("icon-menu").find(".tooltip").remove().end().insertBefore(c).wrap('<div id="cart-link-mobile" />');!f&&b("#cart-link-mobile").append(h);e.on("click",function(b){b.preventDefault();a.is("#cart-link")&&a.trigger("click")});if(!f){var m;n();b(window).on("resize",function(){clearTimeout(m);m=setTimeout(n,300)})}}})(b("#cart-icon-count > a"),b("#menu-icon"))});a(window).load(function(){"undefined"!==
typeof ThemifyGallery&&ThemifyGallery.init({context:jQuery(themifyScript.lightboxContext)});a(function(a){a("#main-nav li").on("mouseenter mouseleave",function(b){if(a("ul",this).length){var c=a("ul:first",this);b=c.offset().left;var c=c.width(),d=a(window).width();b+c<=d?a(this).removeClass("edge"):a(this).addClass("edge")}})})}).on("load debouncedresize ready",function(b){b=a(window).width();var c=a("body");if(c.hasClass("header-logo-center"))1183<b?(b=a(window).width()/2-a("#site-logo").width()/
2,a("#main-nav").css("max-width",b)):a("#main-nav").removeAttr("style");else if(c.hasClass("header-slide-right")||c.hasClass("header-slide-left")){var c=a(".top-icon-wrap, .search-button"),e=a("#mobile-menu"),d=a("#main-nav-wrap");b>tf_mobile_menu_trigger_point?e.before(c):d.before(c)}});var e=function(){a(window).width()>tf_mobile_menu_trigger_point?a("#main-nav li.has-mega-column > ul, #main-nav li.has-mega-sub-menu > .mega-sub-menu").css("width",a("#header").width()):a("#main-nav li.has-mega-column > ul, #main-nav li.has-mega-sub-menu > .mega-sub-menu").removeAttr("style")};
a(document).on("ready",e);a(window).on("debouncedresize",e);e=function(){var b,c=a("#footerwrap"),e=c.find("#footer"),d=c.innerHeight(),f=a("#body"),k=function(){d=c.innerHeight();!p&&c.parent().css("padding-bottom",d)},m=function(){var a=f.get(0).getBoundingClientRect(),b=window.innerHeight-a.bottom;c.toggleClass("active-revealing",0>a.top);0<=b&&b<=d?e.css("opacity",b/d+.2):b>d&&e.css("opacity",1)};if(c.length||f.length){var l=f.parents();l.length&&f.add(l).each(function(){if(!b){var c=a(this).css("background-color");
c&&"transparent"!==c&&"rgba(0, 0, 0, 0)"!==c&&(b=c)}});f.css("background-color",b||"#ffffff");var p="sticky"===c.css("position");a("body").toggleClass("no-css-sticky",!p);k();m();a(window).on("resize",k).on("scroll",m)}};if(a("body").hasClass("revealing-footer"))a(document).on("ready",e)})(jQuery);
